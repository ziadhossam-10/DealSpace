...
export default function EmailTemplates() {
  ...
  // API Calls
  const { data, isLoading, error, refetch } = useGetEmailTemplatesQuery({ page, per_page: pageSize })
  const [deleteEmailTemplate, { isLoading: isLoadingDelete }] = useDeleteEmailTemplateMutation()
  const [bulkDeleteEmailTemplates, { isLoading: isLoadingBulkDelete }] = useBulkDeleteEmailTemplatesMutation()
  ...
  const columns: Column[] = [
      ...
      render: (row: EmailTemplate) => (
        <div className="flex items-center space-x-2">
          <div>
            ...
            <div className="text-sm text-gray-500 truncate max-w-xs">{row.subject}</div>
          </div>
        </div>
      ),
      isMain: true,
    ...
    },
    {
      key: "owner",
      label: "Owner",
      render: (row: EmailTemplate) => <div>{row.user?.name || "-"}</div>,
      isMain: true,
    },
    {
      key: "created_at",
      label: "Created At",
      render: (row: EmailTemplate) => <div>{row.created_at ? new Date(row.created_at).toLocaleDateString() : "-"}</div>,
      isMain: false,
    },
    {
      key: "actions",
  ...
  const handleConfirmDelete = async () => {
    if (selectedTemplateId) {
      try {
        const response = await deleteEmailTemplate(selectedTemplateId).unwrap()
        if (response && response.status) {
          toast.success(response.message || "Email template deleted successfully")
          setShowDeleteModal(false)
          setSelectedTemplateId(null)
          refetch()
        } else {
          toast.error(response.message || "Error deleting email template")
        }
      } catch (error: any) {
        toast.error(error.data?.message || "An error occurred while deleting the email template")
      }
    }
  }
  ...
  const handleConfirmBulkDelete = async () => {
    try {
      const response = await bulkDeleteEmailTemplates({
      ...
      if (response && response.status) {
        toast.success(response.message || `Email templates deleted successfully`)
        ...
        toast.error(response.message || "Error deleting email templates")
      }
    } catch (error: any) {
      toast.error(error.data?.message || "An error occurred while deleting email templates")
    }
  }
    ...
    <AdminLayout>
      <div className="p-6">
        ...
        <DynamicTable
          ...
          currentPage={page}
          idField="id"
          noDataTilte="No Email Templates Found"
          noDataDescription="Create your first email template to get started"
          error={error}
          isLoading={isLoading}
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="p-3 bg-gray-50 rounded-md">{selectedTemplate.subject}</div>
                  </div>
                  <div>
