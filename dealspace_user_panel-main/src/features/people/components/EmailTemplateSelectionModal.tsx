...
import { useGetEmailTemplatesQuery } from "../../emailTemplates/emailTemplatesApi"
...
const EmailTemplateSelectionModal: React.FC<EmailTemplateSelectionModalProps> = ({
  isOpen,
  onClose,
  onSelect,
  initialSearchValue = "",
  ...
  // API Query
  const { data, isLoading, error } = useGetEmailTemplatesQuery({ page, per_page: pageSize, search: searchValue })
  ...
  const columns = [
    ...
    },
    {
      key: "owner",
      label: "Owner",
      render: (row: EmailTemplate) => <div className="text-sm">{row.user?.name || "-"}</div>,
    },
    {
      key: "actions",
      label: "Actions",
...
export default EmailTemplateSelectionModal
