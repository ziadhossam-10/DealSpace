...
import { useCreateTrackingScriptMutation } from "./trackingScriptsApi"
import type { CreateTrackingScriptRequest } from "../../types/trackingScripts"
import FieldMappingHelper from "./FieldMappingHelper"

interface CreateTrackingScriptModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

export default function CreateTrackingScriptModal({ isOpen, onClose, onSuccess }: CreateTrackingScriptModalProps) {
  const [createScript, { isLoading }] = useCreateTrackingScriptMutation()
  const [enableDomainRestrictions, setEnableDomainRestrictions] = useState(false)

  const [formData, setFormData] = useState<CreateTrackingScriptRequest>({
    name: "",
    description: "",
    domain: [""],
    track_all_forms: true, // Changed to true
    form_selectors: [""],
    field_mappings: {
      name: [""],
      first_name: [""],
  ...
  const handleArrayChange = (field: string, index: number, value: string) => {
    const newArray = [...(formData[field as keyof CreateTrackingScriptRequest] as string[])]
    newArray[index] = value
    handleChange(field, newArray)
  }

  const addArrayItem = (field: string) => {
    const currentArray = formData[field as keyof CreateTrackingScriptRequest] as string[]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await createScript(submitData).unwrap()
      if (response.status) {
      ...
      toast.error(error.data?.message || "Failed to create tracking script. Please try again.")
    }
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Create New Tracking Script</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            >
              {isLoading ? "Creating..." : "Create Tracking Script"}
            </button>
          </div>
        </form>
      </div>
