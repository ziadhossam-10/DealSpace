...
import { useCreateAppointmentTypeMutation, useGetAppointmentTypesQuery } from "./appointmentTypesApi"
...
export default function CreateAppointmentTypeModal({ isOpen, onClose }: CreateAppointmentTypeModalProps) {
  const [createAppointmentType, { isLoading }] = useCreateAppointmentTypeMutation()
  ...
  const getNextSortOrder = () => {
    ...
    if (appointmentTypes.length === 0) return 1
    return Math.max(...appointmentTypes.map((dt) => dt.sort)) + 1
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.data?.message || "Failed to create appointment type. Please try again.")
    }
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-lg mx-auto pt-4">
      <div className="flex justify-between items-center p-4 border-b">
        <h3 className="text-lg font-semibold">Create New Appointment Type</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4">
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          >
            {isLoading ? "Creating..." : "Create Appointment Type"}
          </button>
        </div>
      </form>
    </Modal>
